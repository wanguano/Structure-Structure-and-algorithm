...
const linkedList = new ListNode<number>()
linkedList.append(1)
linkedList.append(2)
linkedList.append(3)
linkedList.append(4)
...
function reverseList<T>(oldLinkedList: ListNode<T>): ListNode<T> | null {
  // 题解1: 便利传递的链表,目标每次只取最后一个值(-i-1),然后追加到新的链表中
  let newLinkedList: ListNode<T> = new ListNode<T>()
  // 目标每次拿到最后一个节点,然后删除最后一个节点
  if (oldLinkedList.length < 0) return null

  // 如果链表中就一个元素,则不用反转(2个以上反转才有意义)
  for (let i = 0; i < oldLinkedList.length; i++) {
    let lastElement: T = oldLinkedList.get(oldLinkedList.length - i - 1) as T
    newLinkedList.append(lastElement)
  }
